        ...
        for (let set of JP_SETS) {
            ...
            switch (mode) {
                ...
                case 'fix':
                    ...
                    let existingData = JSON.parse(existingString);
                    for (let parsedCard of formattedCards) {
                        let existingResult = existingData.filter(x => parsedCard.code.indexOf(x.code) > -1);
                        if (existingResult) {
                            for (let result of existingResult) {
                                if (result.ability.length > 0) {
                                    parsedCard.ability = result.ability;
                                }
                                if (result.attributes.length > 0) {
                                ...
                                if (result.jpAbility.length > 0) {
                                    parsedCard.jpAbility = result.jpAbility;
                                }
                            }
