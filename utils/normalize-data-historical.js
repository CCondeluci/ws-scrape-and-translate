...
(async () => {
    ...
    for (let set of EN_SETS) {
        ...
        let bushiString = fs.readFileSync('./full_db/' + set.substring(3) + '.json', 'utf8');
        ...
        finalCards.sort((a, b) => (a.code > b.code) ? 1 : -1);
        
        // write data to file
        fs.writeFileSync('./full_db_fixed/' + set.substring(3) + ".json", JSON.stringify(finalCards, null, 4));
        console.log("-----------------------");
