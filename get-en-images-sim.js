            ...
            cardImagePromiseArr.push(new Promise(async function (resolve, reject) {
                ...
                if (body) {
                    ...
                    if (card.image.indexOf('https://en.ws-tcg.com/cardlist/cardimages') > -1) {
                        ...
                        // if (card.type === 'Climax') {
                        //     sharp(imgBuffer).resize(height, width).png().toFile('./images/' + card.side + card.release + '/' + card.code.replace('/','_').replace('-','_') + '.jpg');
                        // } else {
                            sharp(imgBuffer).resize(width, height).png().toFile('./images/' + card.side + card.release + '/' + card.code.replace('/','_').replace('-','_') + '.jpg');
                        // }
                    }
